...
export class OutboxRepository extends BaseRepository<typeof outbox> implements IOutboxRepository {
    ...
    async getUnprocessedEntries(maxRetries = 5): Promise<OutboxRecord[]> {
        ...
        const entries = await db
            .select()
            .from(this.table)
            .where(and(eq(this.table.processed, 0), lt(this.table.retries, maxRetries)))
            .execute();
        return entries as unknown as OutboxRecord[];
    }

    async claimUnprocessedEntries(limit = 100, maxRetries = 5): Promise<OutboxRecord[]> {
        const db = this.getDb();
        ...
        for (const candidate of candidates) {
            const [locked] = await db
                .update(this.table)
                .set({ processed: 2 })
                .where(and(eq(this.table.id, candidate.id), eq(this.table.processed, 0)))
                .returning()
                .execute();
