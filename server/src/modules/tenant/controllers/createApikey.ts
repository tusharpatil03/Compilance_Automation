...
export async function createApiKey(req: Request, res: Response) {
  try {
    ...
    let encryptedApiKey;
    try {
      encryptedApiKey = encryptData(api_key);
    } catch (error) {
      console.error('Encryption error details:', error);
      console.error('Error encrypting API key:', api_key.length);
      return sendErrorResponse(
        res,
        new ApiError(ErrorCode.INTERNAL_ERROR, 'Failed to encrypt API key', 500)
      );
    }
    ...
    if (error?.message?.includes('already exists')) {
      return sendErrorResponse(
        ...
        new ApiError(
          ...
          `API key with ID '${req.body?.kid}' already exists for this tenant`,
          409,
          'kid'
        )
      );
    ...
    if (error?.message?.includes('Tenant')) {
      return sendErrorResponse(
        ...
        new ApiError(
          ...
          `Tenant with ID ${req.body?.tenant?.id} not found`,
          404
        )
      );
    }
