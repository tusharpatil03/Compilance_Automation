...
export const ApiKeyStatus = pgEnum('apikey_status', [
  'active',
  'inactive',
  'revoked',
]);
...
export const tenants_api_key = pgTable(
  'tenants_api_keys',
  {
    ...
    rotated_from_key_id: integer().references(
      (): AnyPgColumn => tenants_api_key.id
    ),
  },
  (table) => {
    return {
      // composite unique key_prefix to prevent duplicate key prefixes within the same tenant
      uniqueKey: uniqueIndex('tenant_keyprefix').on(
        table.tenant_id,
        table.key_prefix
      ),
