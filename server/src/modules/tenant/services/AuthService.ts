...
export class AuthService {
  async registerTenant(
    uow: UnitOfWork,
    payload: TenantRegisterInput
  ): Promise<{ tenant: TenantResponse; token: string }> {
    // Check if tenant already exists
    return uow.execute(async (uow) => {
      const tenantRepository = uow.getRepository(TenantRepository);

      const existingTenant = await tenantRepository.getTenantByEmail(
        payload.email
      );
      if (existingTenant) {
  ...
  async loginTenant(
    ...
    return uow.execute(async (uow) => {
      ...
      // Verify password
      const isPasswordValid = security.comparePassword(
        payload.password,
        tenant.password,
        tenant.salt
      );
