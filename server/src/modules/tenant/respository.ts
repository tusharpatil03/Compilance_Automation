...
export type IUpdateTenantPayload = Omit<
  Partial<NewTenant>,
...
interface ITenantApiKeyRepository {
  ...
  getActiveApiKeyByKeyPrefix(
    key_prefix: string,
    tenantId: number
  ): Promise<TenantApiKey | null>;
}

class TenantApiKeyRepositoryBase extends BaseRepository<
  typeof tenants_api_key
> {}

export class TenantAPIKeyRepository
  extends TenantApiKeyRepositoryBase
  implements ITenantApiKeyRepository
{
  constructor(db: DrizzleClient) {
    ...
    const [apiKey] = await db
      ...
      .where(
        and(
          ...
          eq(this.table.tenant_id, tenant_id)
        )
      )
      .limit(1)
      .execute();
  ...
  async getActiveApiKeyByKeyPrefix(
    key_prefix: string,
    tenantId: number
  ): Promise<TenantApiKey | null> {
    const db = this.getDb();
  ...
  implements IWebhookRepository
{
  ...
  async createWebhook(payload: Partial<NewWebhook>): Promise<Webhook> {
    ...
    const [created] = await db
      .insert(this.table)
      .values(payload as NewWebhook)
      .returning();
    return created as unknown as Webhook;
  }

  async getWebhooksByTenantId(
    tenantId: number,
    options?: { limit?: number; offset?: number }
  ): Promise<Webhook[]> {
    const db = this.getDb();
  ...
  async deleteWebhooksByTenantId(tenantId: number): Promise<void> {
    const db = this.getDb();
    await db.delete(this.table).where(eq(this.table.tenant_id, tenantId));
  }
}
