...
export class UserServices {
  public async createUser(uow: UnitOfWork, payload: NewUser): Promise<User> {
    ...
    if (!payload.external_customer_id) {
      throw new ApiError(
        ErrorCode.MISSING_REQUIRED_FIELD,
        'Missing required field: external_customer_id',
        400,
    ...
    return uow.execute<User>(async (uow) => {
      ...
      if (existing) {
        const updatedUser = await userRepository.updateUserForTenant(
        ...
        const risk = await riskProfileRepository.getRiskProfileByUserId(
          updatedUser.id
        );
        if (!risk) {
