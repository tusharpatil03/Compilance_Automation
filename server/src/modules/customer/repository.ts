...
export class UserRepository
  extends BaseRepository<typeof customers>
  implements IUserRepository
{
  ...
  async getUserById(external_id: string): Promise<User | null> {
    ...
    try {
      ...
      const user = await db
        .select()
        .from(customers)
        .where(eq(customers.external_customer_id, external_id))
        .limit(1)
        .execute();
  ...
  async getUserByExternalIdAndTenant(
    external_id: string,
    tenant_id: number
  ): Promise<User | null> {
    const db = this.getDb();
    ...
    const result = await db
      ...
      .where(
        and(
          eq(customers.external_customer_id, external_id),
  ...
  async getcustomers(params: QueryTenantcustomers): Promise<User[]> {
    ...
    const list = await db
      .select()
      .from(customers)
      .where(eq(customers.tenant_id, tenant_id))
      .limit(limit)
      .offset(offset)
      .execute();
...
interface IRiskProfileRepository {
  createRiskProfile(payload: {
    user_id: number;
    risk_score: number;
  }): Promise<void>;
  getRiskProfileByUserId(
    user_id: number
  ): Promise<{ risk_score: number } | null>;
}

class RiskProfileBaseRepository extends BaseRepository<typeof risk_profile> {}

export class RiskProfileRepository
  extends RiskProfileBaseRepository
  implements IRiskProfileRepository
{
  constructor(db: DrizzleClient) {
    super(db, risk_profile);
  }

  async createRiskProfile(payload: {
    user_id: number;
    risk_score: number;
  }): Promise<void> {
    const db = this.getDb();
  ...
  async getRiskProfileByUserId(
    ...
    const result = await db
      .select()
      .from(risk_profile)
      .where(eq(risk_profile.user_id, user_id))
      .limit(1)
      .execute();
    if (result.length === 0) {
      return null;
